      ...
      <div className={`w-64 bg-slate-50 h-screen border-r border-slate-200 flex flex-col fixed left-0 top-0 z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          {menu.map(item => {
            ...
            return (
              <Link key={item.label} to={item.path} onClick={() => setIsOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors ${active ? 'bg-blue-100/50 text-blue-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}>
